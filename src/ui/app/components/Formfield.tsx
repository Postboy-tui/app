...
export const FormField: React.FC<FormFieldProps> = ({ label, value, onChange, placeholder, theme, suggestions = [] }) => {
	...
	return (
		<Box flexDirection="column">
			<Box>
				<Box width={8}><Text color={isFocused ? theme.accent : theme.muted}>{label}:</Text></Box><Box borderStyle="round" borderColor={isFocused ? theme.primary : theme.muted} paddingX={1} flexGrow={1}>{value ? <Text color={isFocused ? theme.white : theme.primary}>{value}</Text> : <Text color={theme.muted}>{placeholder}</Text>}</Box></Box>{showSuggestions && (<Box flexDirection="column" marginLeft={8} borderStyle="round" borderColor={theme.muted}>{filteredSuggestions.filter(s => typeof s === 'string' && s.trim().length > 0).map((s, idx) => (<Box key={s || idx}
